...
app.get("/home", async (req, res) => {
   ...
   const places = await db.all("Select * FROM placestovisit where user_id = ?", req.session.userid);

    res.render("home", { admin, places });
});
...
app.post('/profile/delete', async function (req, res) {
    ...
    if(!admin){
        try {
            await db.run(query, loggedInUserId); // Utfører sletting av brukeren fra databasen.
            console.log('Deleted user with ID:', loggedInUserId); // Logger ID-en til brukeren som ble slettet.
            res.redirect('/');  // Omdirigerer tilbake til admin-siden etter sletting.
        } catch (error) {
            console.error('Error when deleting:', error); // Logger eventuelle feil under sletting.
            res.status(500).send("Unable to delete user.");  // Sender feilmelding hvis sletting feiler.
        ...
        /* res.status(500).send("Du kan ikkje slette adminbruker.");  // Sender feilmelding hvis sletting feiler. */
        res.redirect('/home');  // Omdirigerer tilbake til admin-siden etter sletting.
    }
...
app.get("/add-place", async (req, res) => {
    // Sjekker om du er logga inn
    if (!req.session.loggedin) {
        return res.status(403).send("Du må være logget inn for å redigere ein stad.");
    }

    const admin = req.session.admin;
    
    res.render('add_place', { admin });
...
app.get('/edit-place/:id', async (req, res) => {
    if (!req.session.loggedin) {
        return res.status(403).send("Du må være logget inn for å redigere ein stad.");
    }
  ...
  const admin = req.session.admin;
    ...
    const place = await db.get("SELECT * FROM placestovisit WHERE id = ? AND user_id = ?", [req.params.id, req.session.userid]);
 
    if (!place) {
        return res.status(404).send("'plass' ikkje funnet eller tilgang nektet.");
    }
 
    res.render('edit_place', { place, admin });
});

app.post('/edit-place/:id', async (req, res) => {
    if (!req.session.loggedin) {
        return res.status(403).send("Du må være logget inn.");                
    }
...
app.post('/delete-place/:id', async (req, res) => {
    ...
    const db = await dbPromise;
    const result = await db.run("DELETE FROM placestovisit WHERE id = ? AND user_id = ?", [req.params.id, req.session.userid]);
 
    if (result.changes === 0) {
        return res.status(403).send("Sletting mislyktes – plassen tilhører ikkje deg.");
    }
